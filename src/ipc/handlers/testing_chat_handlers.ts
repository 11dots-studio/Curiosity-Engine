// e.g. [curiosity-engine-qa=add-dep]
// Canned responses for test prompts
const TEST_RESPONSES: Record<string, string> = {
  "add-dep": `I'll add that dependency for you.
  
  <curiosity-engine-add-dependency packages="deno"></curiosity-engine-add-dependency>
  
  EOM`,
  "add-non-existing-dep": `I'll add that dependency for you.
  
  <curiosity-engine-add-dependency packages="@angular/does-not-exist"></curiosity-engine-add-dependency>
  
  EOM`,
  "add-multiple-deps": `I'll add that dependency for you.
  
  <curiosity-engine-add-dependency packages="react-router-dom react-query"></curiosity-engine-add-dependency>
...
export function getTestResponse(prompt: string): string | null {
  const match = prompt.match(/\[curiosity-engine-qa=([^\]]+)\]/);
  if (match) {
