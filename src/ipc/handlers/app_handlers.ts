...
import { getCuriosityEngineAppPath, getUserDataPath } from "../../paths/paths";
...
export function registerAppHandlers() {
  ipcMain.handle("create-app", async (_, params: CreateAppParams) => {
    ...
    const fullAppPath = getCuriosityEngineAppPath(appPath);
    ...
    try {
      ...
      await git.commit({
        ...
        author: {
          ...
          email: "curiosity-engine@example.com",
        },
      });
    } catch (error) {
  ...
  ipcMain.handle("get-app", async (_, appId: number): Promise<App> => {
    ...
    // Get app files
    const appPath = getCuriosityEngineAppPath(app.path);
    let files: string[] = [];
  ...
  ipcMain.handle("list-apps", async () => {
    ...
    return {
      apps: allApps,
      appBasePath: getCuriosityEngineAppPath("$APP_BASE_PATH"),
    };
  });
  ...
  ipcMain.handle(
    ...
    async (
      ...
      return withLock(appId, async () => {
        ...
        const oldAppPath = getCuriosityEngineAppPath(app.path);
        const newAppPath = getCuriosityEngineAppPath(appPath);
  ...
  ipcMain.handle("reset-all", async () => {
    ...
    const dyadAppPath = getCuriosityEngineAppPath(".");
