  ...
  ipcMain.handle("chat:stream", async (event, req: ChatStreamParams) => {
    try {
      ...
      if (testResponse) {
        ...
        let codebaseInfo = "";
        if (updatedChat.app) {
          const appPath = getCuriosityEngineAppPath(updatedChat.app.path);
          try {
      ...
      if (!abortController.signal.aborted && fullResponse) {
        // Scrape from: <curiosity-engine-chat-summary>Renaming profile file</dyad-chat-title>
        const chatTitle = fullResponse.match(
          /<curiosity-engine-chat-summary>(.*?)<\/curiosity-engine-chat-summary>/
        );
        if (chatTitle) {
          await db
            .update(chats)
